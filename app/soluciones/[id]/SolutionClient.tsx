...
export default function SolutionClient({ params }: SolutionClientProps) {
  ...
  const [state, handleSubmit] = useForm("mjkabedg");
  ...
  return (
    <>
      ...
      />
      <section className="section-page-header py-6 py-md-10 fix position-relative">
        <div className="container position-relative z-1">
          <div className="text-start">
            <h3 className="fs-4 fs-md-3">{solucion.titulo}</h3>
            <div className="d-flex flex-wrap align-items-center">
              <Link href="/">
                <p className="mb-0 text-900 fs-7 fs-md-6">Inicio</p>
              </Link>
              <svg className="mx-2 mx-md-3 mt-1" xmlns="http://www.w3.org/2000/svg" width={8} height={13} viewBox="0 0 8 13" fill="none">
                <path className="stroke-dark" d="M1 1.5L6.5 6.75L1 12" stroke="#111827" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
              <p className="text-primary mb-0 fs-7 fs-md-6">Detalle de la Solución</p>
            </div>
          </div>
        </div>
      ...
      <section className="section-services-details pt-60 pt-md-80 pb-60 pb-md-80">
        <div className="container">
          <div className="row">
            <div className="col-lg-8 ps-lg-4 content">
              {/* <img className="rounded-3" src="/assets/imgs/services-details-2/img-1.png" alt="Tucs Digital" /> */}
              {solucion.titulos_sugeridos[0] && (
                <h4 className="fw-bold text-primary mb-3 fs-5 fs-md-4">{solucion.titulos_sugeridos[0]}</h4>
              )}
              <p className="mb-4 fs-6 fs-md-5">{solucion.descripcion}</p>
              {solucion.puntos_clave && solucion.puntos_clave.length > 0 && (
                <div className="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-2 mb-4">
                  {solucion.puntos_clave.map((p, i) => (
                    <div className="col d-flex align-items-center mb-2" key={i}>
                      <span className="d-inline-flex align-items-center justify-content-center rounded-circle me-2 flex-shrink-0" style={{width: 24, height: 24, background: '#34d399'}}>
                        <svg width="14" height="14" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M4.5 8.5L7 11L12 6" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                      </span>
                      <span className="fw-medium text-900 fs-7 fs-md-6">{p}</span>
                    </div>
                  ))}
                </div>
              )}
              <h5 className="pt-4 border-top mb-4 fs-6 fs-md-5">Preguntas frecuentes</h5>
              <div className="accordion">
                {solucion.faq.map((f, i) => (
                  <div key={i} className="border-bottom">
                    <button
                      className={`w-100 d-flex align-items-center py-3 py-md-4 px-0 border-0 bg-transparent text-start ${activeItem === i+1 ? '' : ''}`}
                      style={{ outline: 'none' }}
                      onClick={() => setActiveItem(activeItem === i+1 ? 0 : i+1)}
                      aria-expanded={activeItem === i+1}
                    >
                      <span className="d-inline-flex align-items-center justify-content-center rounded-circle me-2 me-md-3 flex-shrink-0" style={{width: 28, height: 28, background: '#7c3aed', color: '#fff', fontWeight: 700, fontSize: 14}}>{i+1}</span>
                      <span className="fw-bold fs-6 fs-md-5 flex-grow-1">{f.pregunta}</span>
                      <span className="ms-2 flex-shrink-0">
                        <svg width="18" height="18" className="d-md-none" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" style={{transform: activeItem === i+1 ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s'}}>
                          <path d="M6 8L10 12L14 8" stroke="#111827" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                        <svg width="20" height="20" className="d-none d-md-block" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" style={{transform: activeItem === i+1 ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s'}}>
                          <path d="M6 8L10 12L14 8" stroke="#111827" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                      </span>
                    </button>
                    {activeItem === i+1 && (
                      <div className="ps-4 ps-md-5 pb-3 pb-md-4 pe-0">
                        <div className="fs-7 fs-md-6 text-600">{f.respuesta}</div>
                      </div>
                    )}
                  </div>
                ))}
            ...
            <div className="col-lg-4 sidebar mt-4 mt-lg-0">
              ...
              <div className="bg-primary rounded-4 position-relative">
                <div className="p-4 p-md-6 p-lg-7">
                  <h4 className="text-white fs-5 fs-md-4">
                    Impulsando la Experiencia Digital <br className="d-none d-md-block" />
                    para tu Negocio
                  </h4>
                  <Link href="#contacto" className="d-flex align-items-center mt-4 mt-md-6 mb-4 mb-md-6">
                    <img src="/assets/imgs/services-details/icon-contact.svg" alt="Tucs Digital" width="48" height="48" />
                    <div className="ms-3">
                      <span className="text-white mb-0 fs-6 fs-md-5">Contáctanos</span>
                      <h5 className="text-white d-block fs-6 fs-md-5 mb-0">+54 9 11 4471 3445</h5>
                    </div>
                  </Link>
                  <Link href="https://wa.me/5491144713445" target="_blank" rel="noopener noreferrer" className="fw-bold btn text-start bg-white fs-7 fs-md-6 d-flex align-items-center justify-content-between text-primary hover-up w-100">
                ...
                <img className="position-absolute top-0 end-0 d-none d-md-block" src="/assets/imgs/services-details/bg-line-3.png" alt="Tucs Digital" />
              </div>
      ...
      <section id="contacto" className="section-contact-2 position-relative section-padding bg-5">
        <div className="container position-relative z-1">
          <div className="text-center px-3">
            <h5 className="ds-5 fs-4 fs-md-3 mt-2 mt-md-3 mb-3">
              ¿Pensando en un proyecto? <br className="d-md-block d-none" />
              Ponte en contacto con nosotros.
            </h5>
            <span className="fs-6 fs-md-5 fw-medium d-block">Conecta hoy con Tucs Digital a través de los datos abajo o completa el formulario para una respuesta rápida</span>
          </div>
          <div className="row mt-6 mt-md-8">
            <div className="col-lg-10 mx-lg-auto">
              <div className="row">
                <div className="col-lg-6 ps-lg-0 pb-4 pb-lg-0">
                  {state.succeeded ? (
                    <div className="px-3">
                      <h4 className="text-success mb-3 fs-5 fs-md-4">¡Mensaje Enviado!</h4>
                      <p className="fs-6">Gracias por contactarnos. Te responderemos en las próximas 24 horas.</p>
                    </div>
                  ) : (
                    <>
                      <h4 className="fs-5 fs-md-4">Déjanos tu mensaje</h4>
                ...
                <div className="col-lg-6">
                  <div className="ps-lg-6 px-3 px-md-0">
                    <h6 className="fs-6 fs-md-5">Chatea con nosotros</h6>
                    <p className="text-500 fs-7 fs-md-6">Nuestro equipo de soporte está disponible</p>
                    <div className="d-flex align-items-center mb-3 mb-md-4 mb-lg-5">
                      <img src="/assets/imgs/contact-2/icon-1.svg" alt="Tucs Digital" width="24" height="24" className="flex-shrink-0" />
                      <Link className="ms-2 text-decoration-underline text-900 fs-7 fs-md-6" href="https://wa.me/5491144713445" target="_blank" rel="noopener noreferrer">Chat vía WhatsApp</Link>
                    </div>
                    <h6 className="fs-6 fs-md-5">Envíanos un correo</h6>
                    <p className="text-500 fs-7 fs-md-6">Nuestro equipo responderá rápidamente tus consultas</p>
                    <div className="d-flex align-items-center mb-3 mb-md-4 mb-lg-5">
                      <img src="/assets/imgs/contact-2/icon-4.svg" alt="Tucs Digital" width="24" height="24" className="flex-shrink-0" />
                      <Link className="ms-2 text-decoration-underline text-900 fs-7 fs-md-6 text-break" href="mailto:tucsdigital@gmail.com">tucsdigital@gmail.com</Link>
                    </div>
                    <h6 className="fs-6 fs-md-5">¿Necesitas ayuda inmediata?</h6>
                    <p className="text-500 fs-7 fs-md-6">Contáctanos para asistencia inmediata</p>
                    <div className="d-flex align-items-center mb-2">
                      <img src="/assets/imgs/contact-2/icon-5.svg" alt="Tucs Digital" width="24" height="24" className="flex-shrink-0" />
                      <Link className="ms-2 text-decoration-underline text-900 fs-7 fs-md-6" href="tel:+5491144713445">+54 9 11 4471 3445</Link>
                    </div>
                  </div>
                </div>
              </div>
